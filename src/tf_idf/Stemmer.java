...
public class Stemmer{
	...
	private void remove_article(StringBuilder word) {
		...
		if (len > 4) {
			if (word.substring(len - 2, len + 1).equals("য়োন")) {
	...
	private void remove_case(StringBuilder word) {
		...
		if (len > 8) {
			if (word.substring(len - 6, len + 1).equals("য়েদেরকে")) {
		...
		if (len > 6) {
			if (word.substring(len - 4, len + 1).equals("দেরকে")
					|| word.substring(len - 4, len + 1).equals("য়েদের")) {
		...
		if (len > 5) {
			if (word.substring(len - 3, len + 1).equals("েদের")
					|| word.substring(len - 3, len + 1).equals("য়েরা")) {
				word.delete(len - 3, len + 1);
				return;
			}
		} /* end if len >5 */
		if (len > 4) {
			if (word.substring(len - 2, len + 1).equals("দের")||
					...
					||word.substring(len - 2, len + 1).equals("য়ের")) {
				word.delete(len - 2, len + 1);
				return;
			}
